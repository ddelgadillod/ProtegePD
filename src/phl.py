...
#        secuencia = self.primerDG    
    ...
    def primerComb(self):
        ...
        for nucleotido in secuencia:
            if nucleotido in 'ATCG':
                combinaciones = [combinacion + nucleotido for combinacion in combinaciones]
            else:
                #print(nucleotido)
                combinaciones = [combinacion_expandida + nucleotido_expandido
                                 for combinacion_expandida in combinaciones
                                 for nucleotido_expandido in expandir_degeneracion(nucleotido)]
            #print(combinaciones[-1]) 
        primerCombL = combinaciones
        return primerCombL

    def primerBaseFreq2(self):
        from collections import Counter
        return [dict(Counter(combinacion)) for combinacion in self.primerComb()]
    ...
    def primerBaseFreq(self):
        ...
        for combinacion in primerComb:
            contador_bases = Counter(combinacion)
            #degSeq = primerComb[i]
            #degFreq = dict(cntr(degSeq))
            #baseFreq.append(degFreq)
            lista_diccionarios.append(dict(contador_bases))
        baseFreq = lista_diccionarios
        return baseFreq
...
def expandir_degeneracion(degeneracion):
    """
    Expande una degeneración de nucleótidos en todas las combinaciones posibles.
    """
    nucleotidos = {'A': 'A', 'T': 'T', 'C': 'C', 'G': 'G', 'R': 'AG', 'Y': 'CT', 'M': 'AC', 'K': 'GT',
                   'S': 'GC', 'W': 'AT', 'H': 'ACT', 'B': 'GCT', 'V': 'ACG', 'D': 'AGT', 'N': 'ACGT'}

    return [''.join(prod) for prod in product(*(nucleotidos[n] for n in degeneracion))]
